...
DiffPanel = function(variable, quantity)
{
   ...
   if (is.data.frame(vari))
      variDIFF = as.matrix(as.numeric(vari[[1]]))
   else
   ...
   for (i in 1:length(quan))
   {
      ...
      pomDIFF = c(NA, (pomDIFF[2:length(pomDIFF)] - pomDIFF[1:(length(pomDIFF) -
                                                                  1)]))
      DIFF = c(DIFF, pomDIFF)
   } # kon i
   DiffPanelPom = as.matrix(DIFF)
   if (!is.null(row.names(vari)))
      row.names(DiffPanelPom) = row.names(vari)
...
LagPanel = function(variable, quantity)
{
   ...
   quan = quantity
   if (is.data.frame(vari))
      variLAG = as.matrix(as.numeric(vari[, 1]))
   else
   ...
   if (!is.null(row.names(vari)))
      row.names(LagPanelPom) = row.names(vari)
...
BGtest = function(residuals, explvariab, acor.ord)
{
   ...
   stat_F = ((RRSS - URSS) / URSS) * ((nrow(YX_testBG) - ncol(YX_testBG) -
                                          1 - p1) / p1)
   prob_F = 1 - pf(stat_F, p1, ((
      nrow(YX_testBG) - ncol(YX_testBG) - 1 - p1
   )))
...
ConoverMulti = function(residuals, subsample)
{
   ...
   for (i in 1:ncol(SzeregPodzielony))
   {
      ...
      if (nrow(SzeregPodzielony_pom) < nrow(SzeregPodzielony))
         SzeregPodzielony_pom = rbind(SzeregPodzielony_pom, matrix(NA, (
            nrow(SzeregPodzielony) - nrow(SzeregPodzielony_pom)
         ), 1))
   ...
   for (i in 1:ncol(SzeregPodzielony))
   {
      ...
      SzeregStackOdchylen_pom = cbind(SzeregStackOdchylen_pom, matrix(
         i,
         nrow(SzeregStackOdchylen_pom),
         ncol(SzeregStackOdchylen_pom)
      ))
      if (i == 1)
         SzeregStackOdchylen = SzeregStackOdchylen_pom
      if (i != 1)
         SzeregStackOdchylen = rbind(SzeregStackOdchylen, SzeregStackOdchylen_pom)
   } # koniec petli
   ...
   Rangi = cbind(
      SzeregStackOdchylenSort[, 1],
      SzeregStackOdchylenSort[, 2],
      as.matrix(Rangi),
      as.matrix(Rangi ^ 2),
      as.matrix(Rangi ^ 4)
   )
               ...
               const)
{
   ...
   for (i in 1:noN)
   {
      ...
      pomSigma = t((pomdy - pomKsiTeta * pomFi)) %*% pomH %*% (pomdy - pomKsiTeta *
                                                                  pomFi) / nrow(pomW)   # sigma^2 liczona z relacji (11)
      sigma[[i]] = pomSigma
      ...
      pomly = ly[[i]]
      pomNawias2 = as.numeric((pomFi / pomSigma)) * t(pomX) %*% pomH %*% (pomdy -
                                                                             pomFi * pomly)        # skladowa i nawiasu drugiego relacji (9)
      Nawias2[[i]] = pomNawias2

      # liczenie LogL
      suma1 = suma1 + log(2 * pi * pomSigma)
      suma2 = suma2 + (1 / pomSigma) * t((pomdy - pomFi * pomKsiTeta)) %*% pomH %*%
         (pomdy - pomFi * pomKsiTeta)
      ...
      varFirstRowFi = cbind(varFirstRowFi,
                            (t(pomX) %*% pomKsiTeta) * as.numeric(pomFi / pomSigma))
      varFirstRowSR = cbind(varFirstRowSR, (t(pomX) %*% pomW) * as.numeric(pomFi /
                                                                              pomSigma))
      ...
      } else
      {
         ...
         varMixedFiSR = rbind(varMixedFiSR,
                              cbind(pom02VarMixedFiSR, pomVarMixedFiSR))
      } # kon else
   } # kon i
   ...
   row.names(Fi2) = paste(rep("ec", nrow(Fi2)), seq(
      from = 1,
      to = nrow(Fi2),
      by = 1
   ), sep = "")
   row.names(sigma2) = paste(rep("sigma2_", nrow(sigma2)), seq(
      from = 1,
      to = nrow(sigma2),
      by = 1
   ), sep = "")
   ...
   if (is.null(VariableNames))
      VariableNames = paste(rep("VariableName", ncol(dataset)), 1:ncol(dataset), sep =
                               "")
   ...
   for (i in 1:noN)
   {
      ...
      High95SR = as.matrix(pom_paramSR - qnorm(p = 0.025, mean = 0, sd = 1) *
                              pom_bledySR)
      pom_paramSR = data.frame(cbind(
         pom_paramSR,
         pom_bledySR,
         pom_tstatSR,
         probSR,
      ...
      names(pom_paramSR) = c(paste("Coef(i=", i, ")", sep = ""),
                             "StdErr",
                             "z",
                             "P>|z|",
                             "Low95%",
   ...
   for (i in 1:noN)
   {
      ...
      if (const)
         X_testBG = as.matrix(X_testBG[, -ncol(X_testBG)])
      ...
      TestyDiag[i, 1] = R2
      TestyDiag[i, 2] = R2bar
      wBG = BGtest(residuals = reszta01,
                   explvariab = pomX_toBG,
                   acor.ord = acor.ord)
                    ...
                    const)
{
   ...
   optimPMG = list(
      ...
      LR = poBS2$LR,
      SR = poBS2$SR,
      DiagTests = poBS2$DiagTests,
      residuals = poBS2$residuals
   )
} # kon pmg
